...
class BargainIntentTest extends TestCase
{
    ...
    public function test_ask_discount_during_active_negotiation_references_shop_line(): void
    {
        Config::set('bargain.ai.enabled', false);
        Config::set('bargain.counter.concession.enabled', false);
        Config::set('bargain.counter.min_step', 100);
        ...
        $shopLine = (string) $this->getJson('/api/v1/bargain/sessions/'.$sessionId.'?customer_phone='.urlencode($phone))
            ->json('data.session.current_offer');
        $this->assertNotSame('', $shopLine);

        $disc = $this->postJson("/api/v1/bargain/sessions/{$sessionId}/messages", [
            'customer_phone' => $phone,
            'message' => 'AP discount karen',
        ]);
        $disc->assertOk();
        $body = (string) $disc->json('data.assistant_message.body');
        $this->assertStringContainsString('PKR 7000.00', $body);
        $this->assertStringContainsString('PKR '.$shopLine, $body);
        $this->assertStringNotContainsStringIgnoringCase('pehle aapka budget', $body);
    }
