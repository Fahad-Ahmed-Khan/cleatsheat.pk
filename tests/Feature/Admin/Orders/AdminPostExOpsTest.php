<?php

namespace Tests\Feature\Admin\Orders;

use App\Enums\OrderStatus;
use App\Enums\PaymentStatus;
use App\Enums\ShipmentStatus;
use App\Models\Courier;
use App\Models\CourierAccount;
use App\Models\Order;
use App\Models\Shipment;
use App\Models\ShipmentEvent;
use App\Models\ShippingSetting;
use App\Models\User;
use Database\Seeders\ShippingCourierSeeder;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Facades\Http;
use Tests\TestCase;

class AdminPostExOpsTest extends TestCase
{
    use RefreshDatabase;

    private function makeOrder(Courier $courier): Order
    {
        return Order::query()->create([
            'order_number' => 'TR-PX-ADM-'.str_pad((string) random_int(1, 9999), 4, '0', STR_PAD_LEFT),
            'guest_email' => 'buyer@example.com',
            'status' => OrderStatus::Processing,
            'payment_status' => PaymentStatus::Pending,
            'payment_gateway' => 'cod',
            'subtotal' => '1000',
            'discount_total' => '0',
            'shipping_total' => '200',
            'cod_fee' => '0',
            'grand_total' => '1200',
            'shipping_address_snapshot' => [
                'full_name' => 'Buyer',
                'phone' => '03001234567',
                'line1' => 'Street 1',
                'city' => 'Karachi',
            ],
            'preferred_courier_id' => $courier->id,
            'courier_assignment' => 'auto',
        ]);
    }

    public function test_admin_can_download_postex_invoice_pdf(): void
    {
        config(['shipping.endpoints.postex' => 'https://api.postex.pk']);

        $this->seed(ShippingCourierSeeder::class);
        $admin = User::factory()->admin()->create();

        $postex = Courier::query()->where('code', 'postex')->firstOrFail();
        $account = CourierAccount::query()->where('courier_id', $postex->id)->firstOrFail();
        $account->credentials = ['api_token' => 'tok-123'];
        $account->save();

        $order = $this->makeOrder($postex);
        $shipment = Shipment::query()->create([
            'order_id' => $order->id,
            'courier_id' => $postex->id,
            'courier_account_id' => $account->id,
            'status' => ShipmentStatus::Booked,
            'tracking_number' => 'CX-111',
            'receiver_snapshot' => $order->shipping_address_snapshot,
        ]);

        Http::fake(fn ($req) => Http::response('%PDF-1.4', 200, ['Content-Type' => 'application/pdf']));

        $resp = $this->actingAs($admin)->get(route('admin.orders.shipment.postex.invoice', [$order->id, $shipment->id]));

        $resp->assertOk();
        $resp->assertHeader('content-type', 'application/pdf');

        Http::assertSent(fn ($req) => $req->hasHeader('token', 'tok-123'));
    }

    public function test_admin_can_generate_postex_load_sheet_pdf(): void
    {
        config(['shipping.endpoints.postex' => 'https://api.postex.pk']);

        $this->seed(ShippingCourierSeeder::class);
        $admin = User::factory()->admin()->create();

        $postex = Courier::query()->where('code', 'postex')->firstOrFail();
        $account = CourierAccount::query()->where('courier_id', $postex->id)->firstOrFail();
        $account->credentials = ['api_token' => 'tok-123'];
        $account->save();

        $settings = ShippingSetting::current();
        $settings->postex_pickup_address_code = 'WH-MAIN-01';
        $settings->save();

        $order = $this->makeOrder($postex);
        Shipment::query()->create([
            'order_id' => $order->id,
            'courier_id' => $postex->id,
            'courier_account_id' => $account->id,
            'status' => ShipmentStatus::Booked,
            'tracking_number' => 'CX-222',
            'receiver_snapshot' => $order->shipping_address_snapshot,
        ]);

        Http::fake(fn ($req) => Http::response('%PDF-1.4', 200, ['Content-Type' => 'application/pdf']));

        $resp = $this->actingAs($admin)->get(route('admin.orders.postex.load-sheet', $order->id));

        $resp->assertOk();
        $resp->assertHeader('content-type', 'application/pdf');

        Http::assertSent(fn ($req) => $req->method() === 'POST'
            && $req->hasHeader('token', 'tok-123')
            && ($req['pickupAddress'] ?? null) === 'WH-MAIN-01'
            && ($req['trackingNumbers'] ?? null) === ['CX-222']);
    }

    public function test_admin_can_cancel_postex_shipment(): void
    {
        config(['shipping.endpoints.postex' => 'https://api.postex.pk']);

        $this->seed(ShippingCourierSeeder::class);
        $admin = User::factory()->admin()->create();

        $postex = Courier::query()->where('code', 'postex')->firstOrFail();
        $account = CourierAccount::query()->where('courier_id', $postex->id)->firstOrFail();
        $account->credentials = ['api_token' => 'tok-123'];
        $account->save();

        $order = $this->makeOrder($postex);
        $shipment = Shipment::query()->create([
            'order_id' => $order->id,
            'courier_id' => $postex->id,
            'courier_account_id' => $account->id,
            'status' => ShipmentStatus::Booked,
            'tracking_number' => 'CX-333',
            'receiver_snapshot' => $order->shipping_address_snapshot,
        ]);

        Http::fake(fn ($req) => Http::response([
            'statusCode' => '200',
            'statusMessage' => 'Successfully Cancelled',
        ], 200));

        $resp = $this->actingAs($admin)->post(route('admin.orders.shipment.postex.cancel', [$order->id, $shipment->id]));

        $resp->assertRedirect(route('admin.orders.show', $order));
        $this->assertSame(ShipmentStatus::Canceled, $shipment->fresh()->status);
        $this->assertTrue(ShipmentEvent::query()
            ->where('shipment_id', $shipment->id)
            ->where('status', ShipmentStatus::Canceled->value)
            ->exists());

        Http::assertSent(fn ($req) => $req->method() === 'PUT' && $req->hasHeader('token', 'tok-123'));
    }
}
