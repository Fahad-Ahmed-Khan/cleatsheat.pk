...
function categoryHref(extra = {}) {
    return `${route('store.category', props.category.slug)}${queryString(extra)}`;
}

function applyFilters() {
    router.get(route('store.category', props.category.slug), buildQuery(), {
        preserveScroll: true,
        replace: true,
    });
...
function clearFilters() {
    ...
    local.size = '';
    local.availability = '';
    router.get(route('store.category', props.category.slug), {}, { preserveScroll: true, replace: true });
    filterOpen.value = false;
}

function toggleBrand(id) {
    const i = local.brand_ids.indexOf(id);
    if (i === -1) local.brand_ids.push(id);
    else local.brand_ids.splice(i, 1);
}

function toggleColor(id) {
    const i = local.color_ids.indexOf(id);
    if (i === -1) local.color_ids.push(id);
    else local.color_ids.splice(i, 1);
}
        ...
        <div class="mx-auto max-w-6xl pb-28 lg:pb-12">
            <div class="border-b border-stone-100 bg-white px-4 py-6 sm:px-6">
                ...
                <div
                    ...
                    class="prose prose-sm prose-stone mx-auto mt-6 max-w-3xl leading-relaxed"
