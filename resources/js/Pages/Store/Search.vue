...
const props = defineProps({
    ...
    fallbackProducts: { type: Array, default: () => [] },
...
function performApply() {
    router.get(route('store.search'), buildQuery(), {
        preserveScroll: true,
        replace: true,
    });
}
...
function clearFilters() {
    ...
    lastAppliedSignature = filterSignature();
    router.get(route('store.search'), local.q ? { q: local.q } : {}, { preserveScroll: true, replace: true });
    filterOpen.value = false;
}
...
const searchQuery = computed(() => props.searchMeta?.query || local.q || '');
const showFallback = computed(() => !feedItems.value.length && props.fallbackProducts?.length > 0);
const fallbackKind = computed(() => props.searchMeta?.fallback || 'popular');
...
<template>
    ...
    <StoreLayout>
        <div class="store-container pb-28 lg:pb-12">
            ...
            >
                <template v-if="searchMeta?.fallback === 'fuzzy'" #below>
                    <p class="mt-2 text-sm text-stadium-secondary">
                        No exact match — showing close results for “{{ searchQuery }}”.
                    </p>
                </template>
            </StoreShopHeader>
            ...
            <div class="lg:grid lg:grid-cols-[minmax(0,220px)_1fr] lg:gap-5 lg:pt-6">
                ...
                <!-- Grid -->
                <div>
                    <div v-if="!feedItems.length" class="space-y-8">
                        <div class="bg-stadium-muted py-16 text-center text-sm text-stadium-secondary">
                            ...
                            <p class="mt-1 text-xs text-stadium-secondary">
                                Try a different spelling, remove a filter, or browse popular picks below.
                            </p>
                        ...
                        <div v-if="showFallback">
                            <h2 class="mb-4 font-display text-lg font-extrabold tracking-tight text-stadium-ink">
                                {{ fallbackKind === 'popular' ? 'Popular right now' : 'You might like' }}
                            </h2>
                            <div class="store-product-grid--sidebar">
                                <StoreProductCard
                                    v-for="(p, i) in fallbackProducts"
                                    :key="`fb-${p.id}`"
                                    :product="p"
                                    :index="i"
                                />
